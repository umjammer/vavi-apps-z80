...
class IN_r_C extends InstructionsExecutionTestsBase {
    ...
    public void IN_r_C_reads_value_from_port(String reg, byte opcode) {
        ...
        var portNumberHigh = fixture.create(Byte.TYPE);
        ...
        executeCase(opcode, portNumberLow, portNumberHigh, value);
    ...
    public void IN_r_C_sets_SF_appropriately(String reg, byte opcode) {
        ...
        executeCase(opcode, portNumberLow, portNumberHigh, (byte) 0xFE);
        assertEquals(1, registers.getSF().intValue());

        executeCase(opcode, portNumberLow, portNumberHigh, (byte) 0xFF);
        assertEquals(1, registers.getSF().intValue());

        executeCase(opcode, portNumberLow, portNumberHigh, (byte) 0);
        assertEquals(0, registers.getSF().intValue());

        executeCase(opcode, portNumberLow, portNumberHigh, (byte) 1);
        assertEquals(0, registers.getSF().intValue());
    }
    ...
    public void IN_r_C_returns_proper_T_states(String reg, byte opcode) {
        ...
        var portNumberHigh = fixture.create(Byte.TYPE);
        var value = fixture.create(Byte.TYPE);
        var states = executeCase(opcode, portNumberLow, portNumberHigh, value);
        assertEquals(12, states);
    }

    private int executeCase(byte opcode, byte portNumberLow, byte portNumberHigh, byte value) {
        registers.setC(portNumberLow);
        registers.setB(portNumberHigh);
        setPortValue(NumberUtils.createShort(portNumberLow, portNumberHigh), value);
